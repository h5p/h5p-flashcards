          ...
          $('<div class="h5p-solution">' +
            ...
            '<span class="solution-text">' +
              (that.options.cards[index].answer ?
                that.options.showSolutionText + ': <span>' + C.splitAlternatives(that.options.cards[index].answer).join('<span> ' + ',' + ' </span>') + '</span>' :
                '') + '</span>' +
          '</div>').appendTo($card.find('.h5p-imageholder'));
  ...
  C.prototype.enableResultScreen = function () {
    ...
    for (var i = 0; i < this.options.cards.length; i++) {
      ...
      if (!userCorrect) {
        ...
        $resultsAnswer.append('<span class="h5p-correct">' + C.splitAlternatives(card.answer).join('<span> ' + ',' + ' </span>') + '</span>');
      }
